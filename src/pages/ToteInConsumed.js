...
const ToteInConsumed = () => {
  ...
  const fetchTasks = async () => {
    setLoading(true);
    try {
      const response = await axios.post(`http://localhost:7373/fetchTaskDtl/${toteNo}`);
  ...
  const fetchOtherAPIs = async () => {
    try {
      ...
      const response1 = await axios.post(`http://localhost:7373/checkPackedOrders/${toteNo}`);
      setPackingRecords(response1.data);
      console.log(response1.data);
      
      // Fetch open allocation records from the second additional API
      const response2 = await axios.post(`http://localhost:7373/CheckAllocation/${toteNo}`);
  ...
  const handleButtonClick = async () => {
      ...
      // Call the API to update tote status
       await axios.post(`http://localhost:7373/updateTotetoAlloc/${toteNo}`)
       .then((response) => {
        // Handle success
  ...
  return (
    <div className="tote-container container">
      ...
      <button className="fetch-button button" onClick={fetchTasks}>Fetch Tasks</button>
      ...
      {uniqueTaskIds.length > 0 && (
        <div className="open-tasks">
          ...
          <p className="open-tasks-message">There are open tasks with task_ids: {formattedTaskIds}. Please complete the tasks.</p>
        </div>
      )}

      {noOpenTasks && (
        <div className="no-open-tasks">
          <h2 className="no-open-tasks-title">No Open Tasks for Tote {toteNo}</h2>
          {packingRecords.length > 0 && (
            <div className="open-packing-records">
              <h3 className="open-packing-records-title">Open Packing Records:</h3>
          ...
          {allocationRecords.length > 0 && (
            <div className="open-allocation-records">
              <h3 className="open-allocation-records-title">There is open Allocation for this Tote .</h3>
            </div>
          )}
        </div>
      )}
      ...
      {showButton && <button className="move-button button" onClick={handleButtonClick}>Move Tote to Allocated and Pulled</button>}
    </div>
  );
};
